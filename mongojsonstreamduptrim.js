...
function writeAll(res) {
    for (var i = 0, len = res.length; i < len; i++) {
        jsonwriter.write(toFan(res[i]));
    }
}

function allPromises(max, db) {
    var q = [], offset = 0;
    while (offset < max) {
        q.push(db.collection('app_users565c819f3169dd7f607b39c6').find().skip(offset).limit(params.chunkSize).toArray().then(writeAll, console.error));
...
function connect() {
    return mongo.connect(`mongodb://${params.host.name}:${params.host.port}/countly`);
}

function count(db) {
    return db.collection(`app_users${params.appId}`).find().count();
}

function connected(db) {
    count(db).then(function(cnt) {
        Promise.all(allPromises(cnt, db)).then(function() {
            jsonwriter.end();
            db.close();
        });
    }, function(err) {
...
connect().then(connected, console.error);
